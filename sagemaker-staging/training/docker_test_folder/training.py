...
class SaveCheckpoints(keras.callbacks.Callback):
    """
    Saves a checkpoint to S3 after each epoch
    """
    def __init__(self, 
        ...
        self.base_name_checkpoint = base_name_checkpoint # base filename for the checkpoint (arbitrary)
        self.lcl_chkpt_dir = lcl_chkpt_dir # local directory where the model stores the checkpoint
        self.s3_chkpt_dir = s3_chkpt_dir # S3 directory you want to upload the checkpoint to
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use-nbr', type=str, default=False)
...
#     s3_chkpt_dir = "ckpt/pc-tf-custom-container-2021-04-20-05-19-50-592"
    ...
    def define_model(numclasses, xbands_input_shape, output_type, starting_checkpoint=None, lcl_chkpt_dir=None,s3_chkpt_dir=s3_chkpt_dir):
        ...
        xbands_input_shape: shape of your input
        output_type: currently deprecated
        starting_checkpoint: starting checkpoint for training if you're not training from scratch
        lcl_chkpt_dir: local directory where checkpoints are stored
        s3_chkpt_dir: s3 directory where the checkpoints are
        """
        ...
        if pre_trained_model == "resnet50": # if the pre-trained model is standard resnet50
        # introduce a additional layer to get from bands to 3 input channels
            ...
            # This commented-out code contains an attempt to separate the RGB bands and training on them first before adding in the other bands later.
            # (That code was originally written by Shailesh and Midhush.)
            # I couldn't get this to work in Sagemaker but I'll keep the code in case you want to work on it.
            
            #base_model_pre_trained = tf.keras.Sequential([
            #    hub.KerasLayer(os.path.join(training_dir, 'sentinel_resnet_tf'))
            #])
            #
            #base_model = keras.applications.ResNet50(include_top=False,
            #                                         weights=None,
    ...
    #         last_chkpt_path = lcl_chkpt_dir + 'last_chkpt.h5'
        ...
        elif pre_trained_model == "resnet_sentinel": # if the pretrained model is the custom resnet trained on sentinel data
            print('resnet sentinel model')
            onnx_model = onnx.load(os.path.join(training_dir, 'weights_resnet50.onnx'))
            base_model_pretrained = onnx_to_keras(onnx_model, ['s2_img_13bands'])
            model_config = base_model_pretrained.get_config()
            ...
            model_config['layers'][0] = {
                      ...
                      'config': {
                          'batch_input_shape': (None, xbands_input_shape[2], xbands_input_shape[0], xbands_input_shape[1]),
                          'dtype': 'float32',
                          'sparse': False,
                          'name': 'new_input'
                      },
            ...
            new_model = base_model_pretrained.__class__.from_config(model_config, custom_objects={})
            input_l = keras.layers.Input(xbands_input_shape)
            layer1 = keras.layers.Permute((3,2,1),input_shape=xbands_input_shape)(input_l)
            output = new_model(layer1)
            model2 = keras.Model(inputs=input_l, outputs=[output])
            
            for l, bl in zip(model2.layers[2].layers[:-4], base_model_pretrained.layers[:-4]):
                # copy over weights for all layers except the final 4 and freeze them
                try:
                    l.set_weights(bl.get_weights())
                    l.trainable = False
                    print(f'Transferring weights for layer {l.name}')
                except:
                    print(f'Could not transfer weights for layer {l.name}')
            ...
            # let's add a fully-connected layer
            x = Dense(2048, activation='relu')(last_layer)
    ...
    #         last_chkpt_path = lcl_chkpt_dir + 'last_chkpt.h5'
        ...
        key = s3_chkpt_dir
        # Look for checkpoints in the s3_chkpt_dir
        ...
        if h5_files_dict:
            # if checkpoints were found, it means the spot instance was interrupted then restarted, so load the most recent checkpoint file
            ...
            if not os.path.exists(last_chkpt_local_path):
